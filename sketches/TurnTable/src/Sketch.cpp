...
void Sketch::draw()
{
  ...
  Matrix arcbalMatrix(arcBall.getMatrix());
  arcbalMatrix.rotateZ(-rollPos * D2R);

  rollPos += rollVel;
  rollVel += rollAcc;

  auto projectionMatrix = glm::perspective(60 * D2R, windowInfo.width / windowInfo.height, 1.0f, 1000.0f);
  auto mvpMatrix = arcbalMatrix * projectionMatrix;
...
void Sketch::keyDown(int keyCode, int modifiers)
{
  ...
  {
    case KEY_LEFT:
      rollAcc = -font->getDirection() * FORCE;
      break;
